...
<div><x-input-label for="especialidade_id" value="Especialidade" /><select id="especialidade_id" class="mt-1 block w-full rounded-md border-gray-300"><option value="">Selecione (opcional)</option>@foreach($especialidades as $especialidade)<option value="{{ $especialidade->id }}">{{ $especialidade->nome }}</option>@endforeach</select><x-input-error :messages="$errors->get('medico_id')" class="mt-2" /></div>
